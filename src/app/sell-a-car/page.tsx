...
import { Camera, Loader, X, Plus, Loader2, AlertCircle } from 'lucide-react';
import { useRouter } from 'next/navigation';
import AuthModal from '../components/Authmodal';
import { getUserProfile, updateUserProfile, logoutUser, TokenManager, isAuthenticated, createUserCar } from '../services/api';
import type { User as UserType, UpdateProfileData, User } from '../services/api';
...
export default function ListCarPage() {
  ...
  useEffect(() => {
    const checkAuthentication = async () => {
      try {
        if (!isAuthenticated()) {
          setAuthError('Please log in to list your car');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        ...
        } else if (key === 'safetyFeatures' || key === 'comfortFeatures') {
          // Send arrays as comma-separated strings (backend expects this format)
      ...
      // Call the API to create user car
      const newCar = await createUserCar(formDataToSend);
      
      console.log('Car listed successfully:', newCar);
...
if (authError || !isAuthenticated() || !user) {
    ...
    <>
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center bg-white rounded-lg shadow-sm p-8 max-w-md">
          ...
          <p className="text-gray-600 mb-4">
            {authError || 'Please log in to list your car'}
          </p>
          <button
      ...
      <AuthModal
        ...
        onSuccess={() => {
          ...
          // Refresh the page to re-check authentication and fetch user data
  ...
  // Main form content (only rendered if authenticated)
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">Welcome back, {user.name}! Fill out the details below to list your car.</p>
        </div>
              ...
              <div className="mb-3">
                <div className="flex gap-2">
                  <div className="flex-1 relative">
                    <div className="w-full px-3 py-2 border border-gray-300 rounded-lg flex flex-wrap items-center gap-2 focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent min-h-[42px]">
